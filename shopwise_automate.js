...
async function getState() {
  try {
    const res = await axios.post(
      ...
      { message: "__GET_STATE__", type: "status_check" },
      { timeout: 6000 },
    );
    return res.data;
  } catch {
...
async function reportTransaction(tx) {
  try {
    ...
    log(
      `Reported to n8n ledger: ${tx.card} ₹${tx.value}${tx.voucher_code ? " voucher " + tx.voucher_code : ""}`,
    );
  } catch (e) {
...
async function fillOtp(scope, code) {
  const boxes = scope.locator(
    'input[type="password"], input[autocomplete="one-time-code"], input[inputmode="numeric"]',
  );
...
async function findPayFrame(page) {
  ...
  const rx =
    /pay now|saved cards|card ending|no cvv|credit card|enter cvv|one[- ]?time password|otp/i;
  while (Date.now() < end) {
...
async function dumpFrames(page) {
  for (const fr of page.frames()) {
    ...
    const body = (
      ...
      .slice(0, 220);
    log(
      `FRAME [${u}] buttons=${JSON.stringify(btns).slice(0, 200)} body="${body}"`,
    );
  }
}
...
async function clickish(page, rx, timeout = 25000) {
  const loc = page
    ...
    .or(page.locator("button", { hasText: rx }))
    .or(page.getByText(rx));
...
async function selectDenominations(page, brandProductId, denominations) {
  ...
  await page
    .getByRole("button", { name: /Increase by one/i })
    .first()
  ...
  for (const b of minusBtns) {
    await b.click().catch(() => {});
    await sleep(350);
  }
  await page
    .screenshot({ path: "shopwise-card-stage.png", fullPage: true })
    .catch(() => {});
  ...
  if (!/[1-9]/.test(label.replace(/buy now with/i, "")))
    throw new Error(
      "Checkout total is ₹0 after qty nudge — order not registered",
    );
  ...
  const limitBanner = await page
    .getByText(/reached this month'?s limit/i)
    .first()
    .isVisible({ timeout: 1500 })
    .catch(() => false);
  const buyDisabled = await buyNow
    .first()
    .isDisabled({ timeout: 1500 })
    .catch(() => false);
  if (limitBanner || buyDisabled) {
    const msg = (
      ...
      .trim();
    throw new Error(
      `MONTHLY_LIMIT_REACHED — ${msg || "Buy-now is disabled (monthly brand limit hit). Resets on the 1st, or ~30 min after a failed/cancelled attempt."}`,
    );
  }
  return buyNow;
}
...
async function payWithSavedCard(page, cardIndex) {
  ...
  for (let i = 0; i < ctx.pages().length; i++) {
    await ctx
      .pages()
      [i].screenshot({ path: `gateway-${i}.png`, fullPage: true })
      .catch(() => {});
  }
  ...
  if (frame && !MANUAL) {
    try {
      ...
      const payBtn = frame
        ...
        .or(frame.getByText(/^\s*pay now/i));
      ...
      log(
        `PayU auto-step incomplete (${e.message}). Please complete card selection + PAY NOW in the window.`,
      );
    }
  } else {
    log(
      "Complete the PayU step in the window: pick your card → PAY NOW → SafeKey.",
    );
  }

  // SafeKey 3DS: fetch a FRESH payment OTP (created after we triggered pay).
  const otp = await waitForOtp("payment_otp", {
    timeoutMs: 240000,
    sinceServer,
  }).catch((e) => {
    log(e.message);
  ...
  if (otp && !MANUAL) {
    ...
    log(
      filled
        ? "3DS: auto-filled OTP ✓"
        : "3DS: could not locate the OTP field automatically",
    );
  } else if (otp) {
    ...
    for (const p of ctx.pages()) {
      ...
      for (const fr of [p, ...p.frames()]) {
        ...
        if (
          ...
          const submit = fr
            .getByRole("button", {
              name: /submit|continue|verify|confirm|pay|proceed/i,
            })
            .or(fr.locator("input[type=submit]"));
          if (
            await submit
              .first()
      ...
      if (!submitted && /safekey|americanexpress|acs|3ds|otc/i.test(p.url())) {
        try {
          ...
          const BROAD =
            "input[type=text], input[type=tel], input[type=number], input[type=password], input[inputmode=numeric], input:not([type])";
          for (const fr of [p, ...p.frames()]) {
            ...
            if (!v.replace(/\D/g, "").includes(otp)) {
              ...
              await box
                .pressSequentially(otp, { delay: 140 })
                .catch(async () => {
                  await p.keyboard.type(otp, { delay: 140 });
                });
              await sleep(500);
            ...
            if (v.replace(/\D/g, "").includes(otp)) {
              await p
                .screenshot({ path: "safekey-filled.png" })
                .catch((e) => log("safekey screenshot failed: " + e.message));
              ...
              const sub = fr
                .getByRole("button", {
                  name: /^\s*(submit|verify|confirm|proceed|continue|pay( now)?)\s*$/i,
                })
                .or(fr.locator("input[type=submit], button[type=submit]"));
              ...
              if (nSub) {
                ...
                log(
                  `SafeKey: clicked submit (${nSub} candidate${nSub > 1 ? "s" : ""})`,
                );
              } else {
                await box.press("Enter").catch(() => {});
                log("SafeKey: no submit button found, pressed Enter");
              }
              submitted = true;
              await sleep(4000);
              const t2 = (
          ...
          if (ctx.pages().some((pg) => /order-confirmation/i.test(pg.url())))
            return true;
        } catch (e) {
          log("3DS attempt failed: " + e.message);
        }
      }
    }
    if (!dumped && Date.now() > deadline - 113000) {
...
async function dumpAllPages(ctx) {
  ...
  for (const p of ctx.pages()) {
    ...
    for (const fr of p.frames()) {
      ...
      if (inputs.length)
        log(
          `  frame [${fr.url().slice(0, 60)}] inputs=${JSON.stringify(inputs).slice(0, 350)}`,
        );
    }
  }
}
...
async function waitForConfirmation(ctx, timeoutMs) {
  ...
  const urlRx =
    /order-confirmation|order[-_]?success|payment[-_]?success|order[-_]?placed|\/(success|confirmation|thank[-_]?you)\b/i;
  while (Date.now() < end) {
    for (const p of ctx.pages()) {
      try {
...
const OTP_FIELD =
  'input[type="password"], input[autocomplete="one-time-code"], input[name*="otp" i], input[id*="otp" i], input[placeholder*="otp" i], input[placeholder*="code" i], input[inputmode="numeric"], input[type="tel"]';
...
async function buyVoucher(
  ...
  if (!productId)
    throw new Error(
      `Unknown brand "${job.brand}" — add its productId to the plan's brands{}`,
    );
  const cardIndex = cardsMap[job.card];
  if (cardIndex == null)
    throw new Error(
      `Unknown card "${job.card}" — map it in the plan's cards{}`,
    );
  const total = (job.denominations || []).reduce(
    (s, d) => s + d.amount * d.qty,
    0,
  );
  ...
  if (dryRun) {
    await page
      .screenshot({ path: `shopwise-dryrun-${Date.now()}.png`, fullPage: true })
      .catch(() => {});
    log(
      `DRY-RUN: reached "Buy now" for ₹${total} on ${job.brand}/${job.card}. Not paying.`,
    );
  ...
  await buyNow
    ...
    .click({ timeout: 8000 })
    .catch(async () => {
      await buyNow.first().click({ force: true });
    });
...
async function runPlan(plan, { dryRun = false, only = null } = {}) {
  if (!MOBILE || !N8N_BASE_URL)
    throw new Error("Set MOBILE and N8N_BASE_URL in .env");
  ...
  try {
    ...
    for (const job of jobs) {
      ...
      for (let r = 1; r <= reps; r++) {
        ...
        log(`\n=== ${label} ===`);
        try {
          const res = await buyVoucher(page, job, cardsMap, brandsMap, {
            dryRun,
          });
          ...
          await page
            .screenshot({
              path: `shopwise-error-${Date.now()}.png`,
              fullPage: true,
            })
            .catch(() => {});
  ...
  results.forEach((r) =>
    log(
      ` - ${r.job}: ${r.ok ? (r.dryRun ? "dry-run ok" : "OK " + (r.voucher?.code ? "code=" + r.voucher.code : "")) : "FAIL: " + r.error}`,
    ),
  );
  return results;
}
...
if (require.main === module) {
  ...
  if (args.serve) {
    ...
    app.listen(PORT, () =>
      log(
        `ShopWise automation service on :${PORT}  (POST /run {plan|planFile, dryRun, only})`,
      ),
    );
  } else if (args.plan) {
    Promise.resolve()
      .then(() => {
        try {
          return JSON.parse(fs.readFileSync(args.plan, "utf8"));
        } catch (e) {
          throw new Error(`Invalid plan: ${e.message}`);
        }
      })
    ...
    console.log(
      "Usage: node shopwise_automate.js --plan <file.json> [--dry-run] [--only N] | --serve",
    );
    process.exit(1);
  }
}
